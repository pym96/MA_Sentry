...
VelocitySmootherEma::VelocitySmootherEma(ros::NodeHandle* nh):nh_(*nh)
{
    ...
    nh_.param<std::string>("/raw_cmd_topic", raw_cmd_topic, "cmd_vel");
    nh_.param<std::string>("/cmd_topic", cmd_topic, "cmd_vel_filtered");
    nh_.param<double>("/cmd_rate", cmd_rate, 30.0);
    nh_.param<int>("/stop_counter", stop_counter, 3);
    nh_.param<double>("max_accel", max_accel, 5.0);

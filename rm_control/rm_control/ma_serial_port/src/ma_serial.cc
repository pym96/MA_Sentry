...
std_msgs::UInt16 red_outpost_current_HP_msg;
std_msgs::UInt16 blue_outpost_current_HP_msg;
std_msgs::UInt16 game_time_msg;

using ReceivePacket = ma_serial_packet::ReceviePacket;
...
void receiveData() {
    ...
    while (ros::ok()) {
        try {
            ...
            {
                data.resize(sizeof(ReceivePacket) - 1);//data存这么多
                ser.read(data.data(), data.size());//全部读进去
                data.insert(data.begin(), header[0]);//头插进去
                ...
                if (crc16::verify_crc16_checksum(reinterpret_cast<uint8_t*>(packet), sizeof(ReceivePacket))) 
                {
                    ...
                    red_outpost_current_HP_msg.data = packet->red_outpost_HP;
                    blue_outpost_current_HP_msg.data = packet->blue_outpost_HP;
...
int main(int argc, char** argv) {
    ...
    red_outpost_HP_pub.publish(red_outpost_current_HP_msg);
    blue_outpost_HP_pub.publish(blue_outpost_current_HP_msg);

...
RandomExplore::RandomExplore(const std::string& name, const BT::NodeConfiguration& config)
    ...
    goal_point_sub_ = nh_.subscribe<geometry_msgs::PointStamped>("/goal_point", 10, &RandomExplore::goalpoint_callback, this);
    srand(time(NULL));
}

void RandomExplore::goalpoint_callback(const geometry_msgs::PointStamped::ConstPtr& msg){
    fixed_point_.x = msg->point.x;
    fixed_point_.y = msg->point.y;
}
...
BT::NodeStatus RandomExplore::tick() {
    ...
    ROS_INFO("Tick function called: fixed_point_x = %f, fixed_point_y = %f", fixed_point_.x, fixed_point_.y);
    if (!goal_reached_) {
    ...
    float radius = static_cast<float>(rand()) / RAND_MAX * 1;
    geometry_msgs::PointStamped new_goal;
    new_goal.header.frame_id = "vehicle";  // 使用"vehicle"作为参考坐标系
    new_goal.point.x = fixed_point_.x + cos(angle) * radius;
    new_goal.point.y = fixed_point_.y + sin(angle) * radius;
